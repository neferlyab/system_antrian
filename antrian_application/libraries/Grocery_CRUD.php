...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_text_input($field_info,$value)
	{
		if($field_info->extras == 'text_editor')
		{
			...
			if($this->config->text_editor_type == 'minimal'){
				$class_name = 'mini-texteditor';
			}else if($this->config->text_editor_type == 'color'){
				$class_name = 'color-texteditor';
			}else if($this->config->text_editor_type == 'full'){
				$class_name = 'texteditor';
			}else if ($this->config->text_editor_type == 'medium'){
				$class_name = 'medium-texteditor';
			}else{
	...
	protected function get_set_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='chosen-multiple-select' data-placeholder='$select_title'  >";
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}'  name='{$field_info->name}' class='$ajax_or_not_class' data-placeholder='$select_title' >";
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input = '<span class="btn btn-sm btn-primary fileinput-button qq-upload-button" id="upload-button-'.$unique.'" style="'.$uploader_display_none.'">
			<span> <i class="fa fa-upload"></i> Upload '.$field_info->name.'</span>
		...
		$input .= "</a> ";
		$input .= "<a href='javascript:void(0)' id='delete_$unique' class='red'><i class='fa fa-times-circle'></i> ".$this->l('form_upload_delete')."</a> ";
